...
class CFG:
    name = "exp059"
    explain = "deberta-v3-base, add 2021data kunisho data"
    wandb = True
    send_slack = False
    send_notion = True
    competition = "FB3"
    ...
    batch_size=32 # https://www.kaggle.com/c/nbme-score-clinical-patient-notes/discussion/308298
    fc_dropout=0.2
    max_len=2048
    weight_decay=0.01
    #target_size=3
...
class CustomModel(nn.Module):
    ...
    def feature(self, inputs):
        ...
        attention_pool = self.attention_pool(last_hidden_states, inputs['attention_mask'])
        
        #_weighted_pool = self.weighted_pooler(all_hidden_states)
        #weighted_pool = _weighted_pool[:, 0]
        
        #eature = torch.cat([weighted_pool, mean_pool], dim=1)
...
def train_loop(folds, folds0, fold): # ★★★
    ...
    optimizer_parameters = get_optimizer_grouped_parameters(model,
                                                            ...
                                                            layerwise_learning_rate_decay=0.7,
                                                            )
...
def main():
    ...
    data_2021 = pd.read_csv(os.path.join(INPUT, "train_2021pl_3.csv"))
    print(data_2021.shape)
    data_2021 = data_2021[~data_2021["text_id"].isin(train["text_id"].unique())].reset_index(drop=True)
    #data_2021 = data_2021[data_2021["cos_sim"]>0.6].reset_index(drop=True)
    #data_2021 = data_2021.drop("cos_sim",axis=1)
    print(data_2021.shape)
    
    train["is_2022"]=1
    data_2021["is_2022"]=0
    
    #train = pd.concat([train, data_2021]).reset_index(drop=True)
    ...
    # ★★★
    for n, (train_index2, val_index2) in enumerate(Fold.split(data_2021, data_2021[CFG.target_cols])):
        data_2021.loc[val_index2, 'fold'] = int(n)

    train['fold'] = train['fold'].astype(int)
    data_2021['fold'] = data_2021['fold'].astype(int)
    
    train2 = pd.concat([train, data_2021], ignore_index=True)
    ...
    logger.info(f"comment: {CFG.explain}")
