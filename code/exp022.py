...
class CustomModel(nn.Module):
    def __init__(self, cfg, config_path=None, pretrained=False):
        ...
        self.fc = nn.Linear(self.config.hidden_size * 2, 6) # 各pooling を結合しているので
    ...
    def feature(self, inputs):
        ...
        max_pool = self.max_pool(last_hidden_states, inputs['attention_mask'])
        mean_pool = self.mean_pool(last_hidden_states, inputs['attention_mask'])

        feature = torch.cat([max_pool, mean_pool], dim=1)
    ...
    def forward(self, inputs):
        ...
        feature1 = self.dropout1(feature)
        feature2 = self.dropout2(feature)
        feature3 = self.dropout3(feature)
        feature4 = self.dropout4(feature)
        feature5 = self.dropout5(feature)
        
        logits = (feature1+feature2+feature3+feature4+feature5) / 5 
...
def main():
    ...
    if CFG.train:
        ...
        oof_df.to_pickle(os.path.join(EXP_PREDS, 'oof_df.pkl'), protocol=4) # paperspaceのdefault環境がpython3.9なのでkaggleで読み込めるように
