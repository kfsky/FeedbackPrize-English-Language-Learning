...
class CFG:
    name = "exp037"
    explain = "deberta-v3-base 1Dconv"
    wandb = True
    send_slack = False
    send_notion = True
    competition = "FB3"
...
class CustomModel(nn.Module):
    ...
    def feature(self, inputs):
        ...
        last_hidden_state = last_hidden_states.permute(0, 2, 1)
        print(f'Last Hidden State Output Shape: {last_hidden_state.shape}')
        
        cnn_embeddings = F.relu(self.cnn1(last_hidden_state))
        cnn_embeddings = self.cnn2(cnn_embeddings)
        print(f'CNN Embeddings Output Shape: {cnn_embeddings.shape}')
        feature, _ = torch.max(cnn_embeddings, 2)
        print(f'Output Shape: {feature.shape}')
